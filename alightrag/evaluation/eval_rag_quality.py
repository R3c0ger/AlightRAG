#!/usr/bin/env python3
"""
RAGAS Evaluation Script for AlightRAG System
...
Usage:
    ...
    python alightrag/evaluation/eval_rag_quality.py -d my_test.json

    # Specify custom RAG endpoint
    python alightrag/evaluation/eval_rag_quality.py --ragendpoint http://my-server.com:9621
    python alightrag/evaluation/eval_rag_quality.py -r http://my-server.com:9621
...
Results are saved to: alightrag/evaluation/results/
...
class RAGEvaluator:
    ...
    def __init__(self, test_dataset_path: str = None, rag_api_url: str = None):
        ...
        Args:
            ...
            rag_api_url: Base URL of AlightRAG API (e.g., http://localhost:9621)
    ...
    def _display_configuration(self):
        ...
        logger.info("  • AlightRAG API:         %s", self.rag_api_url)
    ...
    async def generate_rag_response(
        ...
        """
        Generate RAG response by calling AlightRAG API.
        ...
        Raises:
            Exception: If AlightRAG API is unavailable.
        """
        try:
            payload = {
                "query": question,
            ...
            raise Exception(
                f"❌ Cannot connect to AlightRAG API at {self.rag_api_url}\n"
                f"   Make sure AlightRAG server is running:\n"
                f"   python -m alightrag.api.lightrag_server\n"
                f"   Error: {str(e)}"
            )
        except httpx.HTTPStatusError as e:
            raise Exception(
                f"AlightRAG API error {e.response.status_code}: {e.response.text}"
            )
            ...
            raise Exception(f"Error calling AlightRAG API: {type(e).__name__}: {str(e)}")
    ...
    async def evaluate_responses(self) -> List[Dict[str, Any]]:
        ...
        logger.info("%s", "=" * 70)
        logger.info("🚀 Starting RAGAS Evaluation of AlightRAG System")
...
async def main():
    ...
    Command-line arguments:
        ...
        --ragendpoint, -r: AlightRAG API endpoint URL (default: http://localhost:9621 or $LIGHTRAG_API_URL)
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="RAGAS Evaluation Script for AlightRAG System",
...
Examples:
  ...
  python alightrag/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="AlightRAG API endpoint URL (default: http://localhost:9621 or $LIGHTRAG_API_URL environment variable)",
        )

        args = parser.parse_args()

        logger.info("%s", "=" * 70)
        logger.info("🔍 RAGAS Evaluation - Using Real AlightRAG API")
        logger.info("%s", "=" * 70)
