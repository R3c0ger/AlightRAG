...
### 📁 AlightRAG Evalua'tion Framework Directory Structure

```
lightrag/evaluation/
├── eval_rag_quality.py      # Main evaluation script
├── sample_dataset.json        # 3 test questions about AlightRAG
...
**Quick Test:** Index files from `sample_documents/` into AlightRAG, then run the evaluator to reproduce results (~89-100% RAGAS score per question).
...
```bash
cd /path/to/AlightRAG
python alightrag/evaluation/eval_rag_quality.py
```

**Specify custom dataset:**
```bash
python alightrag/evaluation/eval_rag_quality.py --dataset my_test.json
```

**Specify custom RAG endpoint:**
```bash
python alightrag/evaluation/eval_rag_quality.py --ragendpoint http://my-server.com:9621
```

**Specify both (short form):**
```bash
python alightrag/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
```

**Get help:**
```bash
python alightrag/evaluation/eval_rag_quality.py --help
```
...
| `--ragendpoint` | `-r` | `http://localhost:9621` or `$LIGHTRAG_API_URL` | AlightRAG API endpoint URL |
...
```bash
python alightrag/evaluation/eval_rag_quality.py --dataset path/to/my_dataset.json
```
...
```bash
python alightrag/evaluation/eval_rag_quality.py -d my_dataset.json -r http://localhost:9621
```

**Absolute path to dataset:**
```bash
python alightrag/evaluation/eval_rag_quality.py -d /path/to/custom_dataset.json
```
...
EVAL_QUERY_TOP_K=10      # OP_K query parameter of AlightRAG
...
`sample_dataset.json` contains 3 generic questions about AlightRAG. Replace with questions matching YOUR indexed documents.
...
### "AlightRAG query API errors during evaluation"

The evaluation uses your configured LLM (OpenAI by default). Ensure:
- API keys are set in `.env`
- Network connection is stable

### Evaluation requires running AlightRAG API

The evaluator queries a running AlightRAG API server at `http://localhost:9621`. Make sure:
1. AlightRAG API server is running (`python lightrag/api/lightrag_server.py`)
2. Documents are indexed in your AlightRAG instance
...
1. Start AlightRAG API server
2. Upload sample documents into AlightRAG  throught  WebUI
...
```
INFO: ======================================================================
INFO: 🔍 RAGAS Evaluation - Using Real AlightRAG API
INFO: ======================================================================
...
INFO:   • AlightRAG API:         http://localhost:9621
INFO:   • Results Directory:    results
INFO: ======================================================================
INFO: 🚀 Starting RAGAS Evaluation of AlightRAG System
...
INFO: -------------------------------------------------------------------------------------------------------------------
INFO: 1    | How does AlightRAG solve the hallucination probl... | 1.0000 |  1.0000 | 1.0000 |  1.0000 | 1.0000 |      ✓
INFO: 2    | What are the three main components required in ... | 0.8500 |  0.5790 | 1.0000 |  1.0000 | 0.8573 |      ✓
INFO: 3    | How does AlightRAG's retrieval performance compa... | 0.8056 |  1.0000 | 1.0000 |  1.0000 | 0.9514 |      ✓
INFO: 4    | What vector databases does AlightRAG support and... | 0.8182 |  0.9807 | 1.0000 |  1.0000 | 0.9497 |      ✓
INFO: 5    | What are the four key metrics for evaluating RA... | 1.0000 |  0.7452 | 1.0000 |  1.0000 | 0.9363 |      ✓
INFO: 6    | What are the core benefits of AlightRAG and how ... | 0.9583 |  0.8829 | 1.0000 |  1.0000 | 0.9603 |      ✓
INFO: ===================================================================================================================
INFO:
INFO: ======================================================================
