#!/usr/bin/env python3
"""
LLM Query Cache Cleanup Tool for AlightRAG
...
Usage:
    python -m alightrag.tools.clean_llm_query_cache
    # or
    python alightrag/tools/clean_llm_query_cache.py
...
from alightrag.kg import STORAGE_ENV_REQUIREMENTS
from alightrag.kg.shared_storage import set_all_update_flags
from alightrag.namespace import NameSpace
from alightrag.utils import setup_logger

# Load environment variables
load_dotenv(dotenv_path=".env", override=False)

# Setup logger
setup_logger("alightrag", level="INFO")
...
class CleanupTool:
    ...
    def get_storage_class(self, storage_name: str):
        ...
        if storage_name == "JsonKVStorage":
            ...
            from alightrag.kg.redis_impl import RedisKVStorage

            return RedisKVStorage
        elif storage_name == "PGKVStorage":
            from alightrag.kg.postgres_impl import PGKVStorage

            return PGKVStorage
        elif storage_name == "MongoKVStorage":
            from alightrag.kg.mongo_impl import MongoKVStorage
    ...
    async def count_query_caches_pg(self, storage) -> Dict[str, Dict[str, int]]:
        ...
        Returns:
            Dictionary with counts for each mode and cache_type
        """
        from alightrag.kg.postgres_impl import namespace_to_table_name
    ...
    def print_header(self):
        """Print tool header"""
        print("\n" + "=" * 60)
        print("LLM Query Cache Cleanup Tool - AlightRAG")
        print("=" * 60)
    ...
    async def setup_storage(self) -> tuple:
        ...
        if storage_name == "JsonKVStorage":
            ...
            print("  • AlightRAG Server is completely shut down")
            print("  • No other programs are accessing the storage files")
            print("\n" + "=" * 60)

            confirm = (
                input("\nHas AlightRAG Server been shut down? (yes/no): ")
                .strip()
                .lower()
            )
            if confirm != "yes":
                print(
                    "\n✓ Operation cancelled - Please shut down AlightRAG Server first"
                )
                return None, None, None
    ...
    async def run(self):
        ...
        try:
            ...
            from alightrag.kg.shared_storage import initialize_share_data
            ...
            try:
                from alightrag.kg.shared_storage import finalize_share_data
