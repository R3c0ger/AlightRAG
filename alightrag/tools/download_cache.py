...
def download_tiktoken_cache(cache_dir: str = None, models: list = None):
    ...
    # Common models used by AlightRAG and OpenAI
    if models is None:
        models = [
            "gpt-4o-mini",  # Default model for AlightRAG
            "gpt-4o",  # GPT-4 Omni
            "gpt-4",  # GPT-4
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="alightrag-download-cache",
        description="Download cache files for AlightRAG offline deployment",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Download to default location (~/.tiktoken_cache)
  alightrag-download-cache

  # Download to specific directory
  alightrag-download-cache --cache-dir ./offline_cache/tiktoken

  # Download specific models only
  alightrag-download-cache --models gpt-4o-mini gpt-4
...
For more information, visit: https://github.com/HKUDS/LightRAG
    ...
    parser.add_argument(
        "--version", action="version", version="%(prog)s (AlightRAG cache downloader)"
    )

    args = parser.parse_args()

    print("=" * 70)
    print("AlightRAG Offline Cache Downloader")
    print("=" * 70)
