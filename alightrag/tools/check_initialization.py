#!/usr/bin/env python3
"""
Diagnostic tool to check AlightRAG initialization status.

This tool helps developers verify that their AlightRAG instance is properly
initialized before use, preventing common initialization errors.

Usage:
    python -m alightrag.tools.check_initialization
"""
...
from alightrag.base import StoragesStatus


async def check_lightrag_setup(rag_instance: AlightRAG, verbose: bool = False) -> bool:
    """
    Check if a AlightRAG instance is properly initialized.

    Args:
        rag_instance: The AlightRAG instance to check
    ...
    print("🔍 Checking AlightRAG initialization status...\n")

    # Check storage initialization status
    if not hasattr(rag_instance, "_storages_status"):
        issues.append("AlightRAG instance missing _storages_status attribute")
    ...
    try:
        from alightrag.kg.shared_storage import get_namespace_data
    ...
    if issues:
        ...
        print("  from alightrag.kg.shared_storage import initialize_pipeline_status")
        ...
        print("✅ AlightRAG is properly initialized and ready to use!")
...
async def demo():
    ...
    from alightrag.llm.openai import openai_embed, gpt_4o_mini_complete
    from alightrag.kg.shared_storage import initialize_pipeline_status

    print("=" * 50)
    print("AlightRAG Initialization Diagnostic Tool")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check AlightRAG initialization status")
    parser.add_argument(
